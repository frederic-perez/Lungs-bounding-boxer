...
FindLungs(const ImageCT& a_imgCT, ImageBinary::Pointer& a_imgLabels)
{
  ...
  using BinaryThresholdType = itk::BinaryThresholdImageFilter<ImageCT, ImageBinary>;
  ...
  try {
    ...
    std::cerr << __func__ << ": thresholdFilter->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }
  ...
  using ShapeLabelFilter = itk::BinaryImageToShapeLabelMapFilter<ImageBinary, LabelMap3DType>;
  ...
  try {
    ...
    std::cerr << __func__ << ": featureExtractor->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }
  ...
  std::clog << __func__ << ": #labels after featureExtractor: " << numLabels << std::endl;

  // Remove labels with volume smaller than lambda ml
  //
  const double lambda = 200000.;
  std::clog << __func__ << ": Removing labels with volume smaller than lambda (" << lambda << ") mm^3..." << std::endl;
  ...
  try {
    ...
    std::cerr << __func__ << ": filterLowVolume->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }

  numLabels = filterLowVolume->GetOutput()->GetNumberOfLabelObjects();
  std::clog << __func__ << ": #labels after small blobs removal: " << numLabels << std::endl;

#undef LABEL_DEBUGGING_20160204
#ifdef LABEL_DEBUGGING_20160204
  using ImageWriterLabels = itk::ImageFileWriter<ImageLabels>;
  if (numLabels < std::numeric_limits<ImageWriterLabels::InputImagePixelType>::highest()) {
    std::clog << __func__ << ": Writting Intermediate Label Image" << std::endl;
    ...
    try {
      ...
      std::cerr << __func__ << ": writer->Update: ITK Exception caught: " << e << std::endl;
      return false;
    }
  }
#endif
  ...
  using KeepNObjectsFilter = itk::ShapeKeepNObjectsLabelMapFilter<LabelMap3DType>;
  ...
  keepNObjectsFilter->SetAttribute(KeepNObjectsFilter::LabelObjectType::NUMBER_OF_PIXELS_ON_BORDER);
  try {
    keepNObjectsFilter->Update();
  } catch (itk::ExceptionObject& e) {
    std::cerr << __func__ << ": keepNObjectsFilter->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }

  std::clog << __func__ << ": Number of labels found: " << keepNObjectsFilter->GetNumberOfObjects() << std::endl;

  using LabelMapToLabelImageFilterType = itk::LabelMapToLabelImageFilter<LabelMap3DType, ImageBinary>;
  LabelMapToLabelImageFilterType::Pointer labelMapToLabelImageFilter = LabelMapToLabelImageFilterType::New();
  labelMapToLabelImageFilter->SetInput(keepNObjectsFilter->GetOutput());
  try {
    labelMapToLabelImageFilter->Update();
  } catch (itk::ExceptionObject& e) {
    std::cerr << __func__ << ": labelMapToLabelImageFilter->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }
  ...
  using ConnectedComponentImageFilterType = itk::ConnectedComponentImageFilter<ImageBinary, ImageBinary>;
  ...
  try {
    ...
    std::cerr << __func__ << ": connected->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }
...
bool
RemoveExternalStuffByFloodFilling(ImageCT::Pointer& a_imgCT, const std::string& a_filenameOutputLabelsSpyHalfWay)
{
  ...
  std::clog << __func__ << ": Bounding box: [" << iMin << ", " << iMax << "] x [" << jMin << ", " << jMax << "] x ["
            << kMin << ", " << kMax << "]" << std::endl;
  ...
  // Region growing
  //
  std::clog << __func__ << ": Setting and executing the region growing filter..." << std::endl;

  using RegionGrower = itk::ConnectedThresholdImageFilter<ImageCT, ImageCT>;
  ...
  try {
    ...
    std::cerr << __func__ << ": regionGrower->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }

  // Mask filter
  //
  std::clog << __func__ << ": Setting and executing a mask filter..." << std::endl;
  ...
  try {
    ...
    std::cerr << __func__ << ": maskFilter->Update: ITK Exception caught: " << e << std::endl;
    return false;
  }

  // Update a_imgCT
  //
  std::clog << __func__ << ": Setting the input/output CT image as the result of the mask filter..." << std::endl;
  ...
  if (!a_filenameOutputLabelsSpyHalfWay.empty()) {
    std::clog << __func__ << ": Writing the output (spy) file as requested..." << std::endl;
...
ComputeAndOutputBoundingBox(const ImageBinary& a_imgLabels)
{
  ...
  std::clog << __func__ << ": Bounding box of non-zero labels: [" << iMin << ", " << iMax << "] x [" << jMin << ", "
            << jMax << "] x [" << kMin << ", " << kMax << "]" << std::endl;
}
...
main(int argc, char* argv[])
{
  ...
  std::clog << __func__ << ": Parsed parameters:\n"
            ...
            << " <filenameOutputLabelsSpyHalfWay> = "
            << (filenameOutputLabelsSpyHalfWay.empty() ? "[empty]" : filenameOutputLabelsSpyHalfWay) << std::endl;
  ...
  try {
    ...
    std::cerr << __func__ << ": inputImage->Update: ITK Exception caught: " << e << std::endl;
    return EXIT_FAILURE;
  }
  ...
  //
  bool succeeded = RemoveExternalStuffByFloodFilling(imageCT, filenameOutputLabelsSpyHalfWay);
  if (!succeeded) {
    std::cerr << __func__ << ": Error: RemoveExternalByFloodFilling did not succeed. Aborting...\n";
    return EXIT_FAILURE;
  }
  ...
  if (!succeeded) {
    std::cerr << __func__ << ": Error: FindLungs did not succeed. Aborting...\n";
    return EXIT_FAILURE;
  }
